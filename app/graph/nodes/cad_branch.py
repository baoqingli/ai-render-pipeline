...
# 工具以 from-import 落名于本模块：测试 monkeypatch 的是
# app.graph.nodes.cad_branch.{inspect_dxf, parse_scene, convert_dwg}（同 mini_render 惯例）
ALLOWED_EXT = {".dwg", ".dxf"}
...
async def parse_node(state: PipelineState) -> dict:
    ...
    result = parse_scene(dxf, state.get("cad_report"))
    if not result.ok or result.data is None:
