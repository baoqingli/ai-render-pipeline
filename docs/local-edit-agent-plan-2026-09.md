...
## 九、实施记录与已知边界（2026-09-15 实施后补充）

已按步骤 1-4 交付（步骤 5 待做）。实施中修正的两个关键点：

1. **定位提示词不能传整条编辑指令**——"定位：把沙发换成深绿色沙发"会让
   VLM 去找"深绿色沙发"（图中不存在的新样子）而乱指 bbox。改为"框出编辑
   会触及的目标（被替换/修改的物体本身，不是它的新样子）"。
2. **qwen grounding 输出 0-1000 归一化坐标**（Qwen-VL 惯例），直接当像素用
   会定位到完全错误的位置，须按图像尺寸反算。

已知边界：

- **移动类指令的遮罩应含目标位置**。理想遮罩 = 源位置 ∪ 目标位置。实测
  "沙发上移贴墙"成功是因为 grounding bbox 恰好覆盖落点；若目标位置远离
  源 bbox（如"移到右边卧室"），合成会把新位置盖回原图，移动被"吃掉"。
  修正方向（迭代项）：①grounding 对移动类指令要求同时框出源与目标（多
  bbox 并集）；②或编辑后对原图/重生成图做显著差异检测自动扩张遮罩。
